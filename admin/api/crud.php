...
try {
    ...
    // ── BORRAR TODOS LOS PARTIDOS DE UNA LIGA ─────────────────
    if ($action === 'delete_partidos_liga') {
        $ligaId = (int)($input['liga'] ?? 0);
        if (!$ligaId) { resp(false, 'ID de liga inválido'); }

        // Igual que delete_old_partidos: limpiar destacados primero (sin FK)
        $stmt = $conn->prepare("
            DELETE FROM partidos_destacados
            WHERE partido_id IN (SELECT id FROM partidos WHERE liga = ?)
        ");
        $stmt->bind_param('i', $ligaId);
        $stmt->execute();
        $stmt->close();

        $stmt = $conn->prepare("DELETE FROM partidos WHERE liga = ?");
        $stmt->bind_param('i', $ligaId);
        $ok       = $stmt->execute();
        $borrados = $stmt->affected_rows;
        $stmt->close();
        ...
        resp($ok, $ok ? "Se eliminaron {$borrados} partidos de esta liga" : 'Error al eliminar');
    }
    ...
    if ($action === 'save' && $entity === 'partido') {
        ...
        // Construir update de canal1..canal10 + extra_min
        $sets   = [];
        $params = [];
        $types  = '';
        ...
        $sets[]   = "extra_min = ?";
        $params[] = max(0, (int)($d['extra_min'] ?? 0));
        $types   .= 'i';
